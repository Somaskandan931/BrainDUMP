...
_PUBLIC_PATHS = {"/api/auth/register", "/api/auth/login", "/api/auth/google", "/health"}
...
def test_the_limiter_window_slides_and_expires():
    from backend.auth.rate_limit import SlidingWindowLimiter
...
def _google_profile(monkeypatch, *, sub="google-sub-123", email="someone@example.com"):
    from backend.api import auth as auth_api
    from backend.auth.google_login import GoogleProfile
