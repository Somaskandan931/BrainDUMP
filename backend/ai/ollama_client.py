...
rename across every file that does `from backend.ai.ollama_client import
...
def _headers() -> dict[str, str]:
    ...
    return {
        ...
        # OpenRouter asks for these on free-tier requests for attribution/
        # rate-limit bucketing; harmless to send, safe to leave generic.
...
def call_model(
    ...
    *,
    ...
    if json_mode:
        # Best-effort: not every free model honors this, which is exactly
        # why call_model_json() below treats invalid JSON as a normal,
        # catchable OllamaError rather than assuming this always works.
