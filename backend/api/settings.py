...
def list_time_blocks(db: Session = Depends(get_db)) -> List[TimeBlockRead]:
...
def create_time_block(payload: TimeBlockCreate, db: Session = Depends(get_db)) -> TimeBlockRead:
...
def delete_time_block(block_id: str, db: Session = Depends(get_db)) -> None:
